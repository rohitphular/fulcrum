...
- Represents a consolidation loan taken out to repay multiple existing debts. Track the consolidated loan itself here; close or archive the individual debts it replaced.
- `opening_balance` represents the current outstanding balance — entered as a positive number (e.g. `18000`); the backend stores it as `−18000`.
...
- `loan_collateral` does not apply to debt consolidation loans (mortgage and auto_loan only).
