...
- `loan_collateral` is not applicable to personal loans and is not shown or editable.
...
- `money-out` from a personal loan account is hard-blocked by Rule 5 in `transaction-validations.md`, except for `Debt & finance / Interest & charges` (capitalised interest).
