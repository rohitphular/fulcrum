...
- **Editable fields:** `name`, `is_active`, `institution`, `account_number_last4`, `notes`, `loan_interest_rate`, `loan_interest_type`, `loan_tenure_months`, `loan_end_date`, `loan_monthly_repayment`, `loan_collateral`.
...
- **`current_balance` is read-only.** Shown as a display field (absolute value, "owed" label). Balance changes via repayment transactions only — record a `money-transfer` from a current/savings account to this mortgage account.
- `money-out` from a mortgage account is hard-blocked by Rule 5 in `transaction-validations.md`, except for `Debt & finance / Interest & charges` (capitalised interest).
- `loan_collateral` is editable for mortgage accounts (e.g. to correct or update property details).
