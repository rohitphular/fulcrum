...
- **`current_balance` is read-only.** Balance changes via repayment transactions only — record a `money-transfer` from a current/savings account to this loan account.
- `money-out` from an auto loan account is hard-blocked by Rule 5 in `transaction-validations.md`, except for `Debt & finance / Interest & charges` (capitalised interest).
- `loan_collateral` is editable for auto loan accounts (e.g. to update vehicle details).
