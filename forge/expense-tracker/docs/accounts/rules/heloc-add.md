...
- HELOC — Home Equity Line of Credit. A revolving credit facility secured against home equity.
- `opening_balance` represents the current outstanding draw — entered as a positive number (e.g. `30000`); the backend stores it as `−30000`.
- `current_balance` starts equal to the negated `opening_balance` at account creation.
- `loan_original_amount` is the original credit line limit at origination and is **immutable after creation**.
- `loan_collateral` does not apply to HELOC (mortgage and auto_loan only).
