...
export async function render(containerId, { txs, sym }) {
  ...
  if (!container) {
    console.warn('[dashboard-25] container not found:', containerId);
    return null;
  }
  ...
  console.log(`[dashboard-25] cities=${rows.length}, domestic=${domTotal.toFixed(0)}, foreign=${foreignTotal.toFixed(0)}`);
