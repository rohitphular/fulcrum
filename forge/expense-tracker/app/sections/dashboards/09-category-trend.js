...
export async function render(containerId, { txs, sym, from, to }) {
  ...
  if (!container) {
    console.warn('[dashboard-09] container not found:', containerId);
    return null;
  }
  ...
  console.log(`[dashboard-09] ${moneyOut.length} txs, ${datasets.length} categories, ${monthKeys.length} months`);
