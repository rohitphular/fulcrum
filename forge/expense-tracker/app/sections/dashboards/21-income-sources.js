...
// and keep state.dashChartInstance in sync (same pattern as 11-category-drilldown).
let _chart = null;
...
export async function render(containerId, { txs, from, to, sym }) {
  _chart = null; // reset stale ref from any previous render of this dashboard

  const container = el(containerId);
  if (!container) {
    console.warn('[dashboard-21] container not found:', containerId);
  ...
  console.log(`[dashboard-21] income_txs=${inTxs.length}, months=${monthKeys.length}`);
