...
import { getCssColors, baseChartOptions } from './dashboard-utils.js';
...
function _render(container, { from, to, sym }) {
  ...
  container.innerHTML = `
    ...
    <div class="dash-tabs" style="margin-bottom:16px">
      ${WINDOWS.map(w =>
        `<button class="dash-tab${_windowDays === w.value ? ' active' : ''}" data-action="eb-window" data-win="${esc(String(w.value))}">${esc(w.label)}</button>`
      ).join('')}
    </div>
  ...
  console.log(`[dashboard-00] income=${currentIncome.toFixed(2)}/day expense=${currentExpense.toFixed(2)}/day savings=${currentSavings.toFixed(2)}/day window=${_windowDays}d`);

  _localChart = _buildChart(canvas, labels, incomeRates, expenseRates, savingsRates, sym);
  state.dashChartInstance = _localChart;
