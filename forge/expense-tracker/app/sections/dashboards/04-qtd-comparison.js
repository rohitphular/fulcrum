...
function _renderTransactions(container, { from, to, sym }) {
  ...
  console.log(`[dashboard-04] transactions — A=${currentQLabel} D=${daysElapsed}, B=${prevQLabel} hasPrev=${hasPrevData}`);
...
function _renderAccounts(container, { from, to, accounts, sym }) {
  ...
  const C = getCssColors();
  console.log(`[dashboard-04] accounts — A=${currentQLabel} D=${daysElapsed}, latestA=${latestA.toFixed(0)}`);
...
export async function render(containerId, { txs, accounts, from, to, sym, tab }) {
  ...
  if (!container) {
    console.warn('[dashboard-04] container not found:', containerId);
    return null;
  }
  if (tab === 'accounts') {
