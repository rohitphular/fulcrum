...
function _renderTransactions(container, { txs, from, to, sym }) {
  ...
  console.log(`[dashboard-01] rendering transactions tab — periodA=${labelA}, periodB=${labelB}, daysA=${daysInMonthA}, daysB=${daysInMonthB}, maxDays=${maxDays}`);
...
function _renderAccounts(container, { accounts, from, to, sym }) {
  ...
  console.log(`[dashboard-01] rendering accounts tab — periodA=${labelA}, periodB=${labelB}, assetAccounts=${assetAccounts.length}`);
...
export async function render(containerId, { txs, accounts, from, to, sym, tab }) {
  ...
  if (!container) {
    console.warn('[dashboard-01] container not found:', containerId);
    return null;
  }
