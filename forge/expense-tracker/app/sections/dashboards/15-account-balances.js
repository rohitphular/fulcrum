...
export async function render(containerId, { accounts, sym }) {
  ...
  if (!container) {
    console.warn('[dashboard-15] container not found:', containerId);
    return null;
  }
  ...
  console.log(`[dashboard-15] assets=${assets.length}, liabilities=${liabilities.length}, investments=${investments.length}, net=${netWorth.toFixed(0)}`);
