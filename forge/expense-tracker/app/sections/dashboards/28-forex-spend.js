...
import { sumAmountBase, getCssColors, buildPalette } from './dashboard-utils.js';
...
export async function render(containerId, { txs, sym }) {
  ...
  const container = el(containerId);
  if (!container) {
    console.warn('[dashboard-28] container not found:', containerId);
  ...
  console.log(`[dashboard-28] currencies=${rows.length}, foreign_gbp=${foreignGbp.toFixed(0)}, has_scatter=${hasScatter}`);
