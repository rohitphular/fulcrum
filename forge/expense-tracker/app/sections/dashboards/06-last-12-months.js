...
function _renderTransactions(container, { sym }) {
  ...
  console.log(`[dashboard-06] transactions — income=${totalIncome.toFixed(0)}, expense=${totalExpense.toFixed(0)}, net=${totalNet.toFixed(0)}`);
...
function _renderAccounts(container, { accounts, sym }) {
  ...
  console.log(`[dashboard-06] accounts — groups=${groupEntries.length}, totalAssets=${totalAssets.toFixed(0)}`);
...
export async function render(containerId, { txs, accounts, from, to, sym, tab }) {
  ...
  if (!container) {
    console.warn('[dashboard-06] container not found:', containerId);
    return null;
  }
  if (tab === 'accounts') {
