...
function _renderTransactions(container, { from, sym }) {
  ...
  console.log(`[dashboard-02] transactions — A=${labelA} total=${totalA.toFixed(0)}, B=${labelB} total=${totalB.toFixed(0)}`);
...
function _renderAccounts(container, { from, accounts, sym }) {
  ...
  const C = getCssColors();
  console.log(`[dashboard-02] accounts — A=${labelA} latest=${latestA.toFixed(0)}, B=${labelB} latest=${latestB.toFixed(0)}`);
...
export async function render(containerId, { txs, accounts, from, to, sym, tab }) {
  ...
  if (!container) {
    console.warn('[dashboard-02] container not found:', containerId);
    return null;
  }
