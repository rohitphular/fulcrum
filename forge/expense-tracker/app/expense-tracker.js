...
function renderCategories() {
  ...
  if (!state.categories.length) {
    el2.innerHTML = `<div class="empty-state"><strong>No categories yet</strong>Run <code>seedCategories()</code> in the Apps Script editor or add rows to the <code>categories</code> tab.</div>`;
    return;
  }
  ...
  const html = types.map(type => {
    ...
    const majors = [...new Set(cats.map(c => c.major_category))];
    const majorHtml = majors.map(major => {
      const minors = cats.filter(c => c.major_category === major).map(c => c.minor_category);
      return `<div class="cat-major">
        <div class="cat-major-name">${esc(major)}</div>
        <div class="cat-minors">${minors.map(m => `<span class="cat-minor">${esc(m)}</span>`).join('')}</div>
      </div>`;
    }).join('');

    return `<div class="cat-section">
      <div class="cat-type-heading">${esc(type)}</div>
      ${majorHtml}
    </div>`;
  }).join('');
