...
**Scope:** All 28 dashboard JS files + coordinator (`dashboard.js`) + `dashboard-utils.js`
**Date:** 2026-08-03
...
### B1 — Dashboard 23: Category column sorts by wrong key
...
### B2 — Dashboard 10: "Previous period" is always the calendar month prior
...
### B4 — Dashboard 15: Liability classification fails silently when schema missing
...
### B5 — Dashboard 26: Loan canvas IDs break if account name contains special chars
...
### I1 — Dashboard 19: Waterfall "Closing" balance excludes transfers
...
### I2 — Dashboard 23: Recurring detection threshold is very strict (5%)
...
### I3 — Dashboard 25: Domestic country is hardcoded to UK
...
### I4 — Dashboard 24 / 25: `COUNTRY_NORM` duplicated
...
**Suggestion:** Move to `dashboard-utils.js` and export as `COUNTRY_NORM`.

---

### I5 — Dashboard 18: Mobile stacked mode visually confusing
...
### I6 — Dashboard 20: Income and expense bars hidden on mobile by default
...
If a dashboard module fails to load (transient network error), `_renderers[dashId]` is set to `null` permanently. The next time the user selects the same dashboard in the same session, it returns `null` immediately without retrying. The user sees "not yet implemented" with no way to recover without a page refresh.

**Fix:** Don't cache `null` — only cache successful imports. On failure, set `_renderers[dashId]` to `undefined` (or delete the key) so the next selection retries the import.

---

### I8 — Dashboard 27: "Transactions" tab is actually income trend, not transactions
...
**Suggestion:** Rename the tab to "Income trend" and update `state.dashTab` handling in the coordinator accordingly (or keep the same `tab: 'transactions'` key but change the visible label).

---

### I9 — Dashboard 15: `_currentBalance` called per-account in a loop (N replay passes)
...
**Dashboard:** 11-category-drilldown
...
**Dashboard:** 19-cashflow-waterfall
...
**Dashboard:** 18-income-vs-expenses
...
**Mobile layout:** All dashboards use `chart-container` which constrains height. On phones, horizontal bar charts with many bars (D22, D23, D24, D25) render correctly because height is dynamic (`rows.length * 44`). The main mobile concern is the stat-cards grid (4 cards on 2 columns) — on very narrow screens (<340px) the 4-up grid wraps awkwardly. Consider a 2×2 grid forced layout for stat cards on mobile.

**Period filter + month-based charts:** Dashboards 09, 13, 14, 16, 17, 18, 20 use `monthRange(from, to)` which produces partial months when `last_7/30/60/90` is selected. The charts render correctly (partial months show partial data) but there's no visual indicator that the first/last month bars are partial. D18 already adds a `*` annotation for the current partial month — consider applying the same to all period-affected charts when a rolling-day filter is active.

**Hardcoded chart colors vs CSS vars:** Dashboards 18, 19, 21, 23 use hardcoded rgba strings for income (green) and expense (red) colours. These are intentional semantic colours (green = income, red = expense) rather than theme palette colours, so they don't need to read from CSS vars. They should remain readable on both light and dark themes — verify contrast on dark mode.
