...
**File:** `sections/dashboards/11-category-drilldown.js`
**Group:** Categories
...
```js
state.dashDrillMajor: string | null
```
...
Unlike other dashboards, this one manages its own Chart.js instance internally because the drill-down click handler must destroy the current chart and create a new one without going through the coordinator.

```js
let _chart = null;

function _destroyChart() { _chart.destroy(); _chart = null; state.dashChartInstance = null; }
function _setChart(c)    { _chart = c; state.dashChartInstance = c; }
```

`state.dashChartInstance` is kept in sync so the coordinator's `_destroyChart()` (called on period change or navigation) still works correctly.
...
3. Chart `onClick` callback: `majors[elements[0].index].cat` → sets `state.dashDrillMajor`, destroys level-1 chart, calls `_renderLevel2`.
...
3. "← Back" button rendered above chart. Click listener: `state.dashDrillMajor = null`, destroy chart, call `_renderLevel1`.
...
| `getCssColors`, `buildPalette`, `baseChartOptions` | `dashboard-utils.js` |
