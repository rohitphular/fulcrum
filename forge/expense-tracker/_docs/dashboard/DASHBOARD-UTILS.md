# dashboard-utils.js — Reference

**Location:** `sections/dashboards/dashboard-utils.js`

Shared computation helpers for all dashboard sub-modules. Import what you need:
...
Replays ALL transactions chronologically from each account's `opening_value`, returning one total-asset-value entry per calendar day in `[from, to]` (inclusive). Used by MoM (01), YoY (02), WoW (03), and Net Worth dashboards.
...
Returns currencies that appear in transactions or accounts but have no entry in `state.rateMap`. Used to render the `.dash-warn` banner.
...
## Usage pattern in a sub-dashboard
