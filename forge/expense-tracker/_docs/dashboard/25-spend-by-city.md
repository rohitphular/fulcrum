...
**File:** `sections/dashboards/25-spend-by-city.js`
...
`money-out` spend grouped by city — granular to location level. Domestic (UK) cities in teal, international in amber. Same structure as Dashboard 24 (country-level) one level deeper.
...
Country normalisation uses the same lookup map as Dashboard 24 (`uk` / `gb` → `United Kingdom`, etc.).
...
Horizontal bar identical in structure to Dashboard 24.
