# Dashboard 06 — Last 12 Months

**File:** `sections/dashboards/06-last-12-months.js`
...
**No comparison period. Fixed window. Mixed chart type.** Unlike the other comparison dashboards, this shows a single 12-month window in absolute terms — not vs a prior period. The coordinator's `from`/`to` are ignored; the window is always computed internally.
...
| `computeDailyTotalAssets` | `dashboard-utils.js` |
