...
**File:** `sections/dashboards/21-income-sources.js`
...
Dashboard 21 manages its own tab strip (not the coordinator's). The pattern mirrors `11-category-drilldown`:
...
`render()` returns `{ destroy() { _destroyChart(); } }`. After any sub-view switch, `_setChart(newChart)` overwrites `state.dashChartInstance` with the new chart directly, so the coordinator always calls `.destroy()` on the currently active chart.

`_chart = null` is set at the top of `render()` to clear any stale reference from a previous render of this dashboard.
...
| `getCssColors`, `baseChartOptions`, `buildPalette` | `dashboard-utils.js` |
