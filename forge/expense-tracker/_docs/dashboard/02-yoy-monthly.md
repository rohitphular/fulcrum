...
**File:** `sections/dashboards/02-yoy-monthly.js`
...
The coordinator passes `from`/`to` from the standard period picker. The sub-dashboard derives both comparison periods from `from`:
...
2. Build cumulative day-by-day arrays via `_buildCumulative` — same algorithm as Dashboard 01.
...
Same as Dashboard 01 accounts tab, using `_computeDailyTotalAssets` with Period A and Period B derived above.
...
- Both periods fetch from `state.transactions` directly — `options.txs` (which is filtered to the selected period by the coordinator) is not used, because the sub-dashboard needs Period B independently.
- `_buildCumulative` and `_computeDailyTotalAssets` are duplicated from `01-mom-cumulative.js`. Extract to `dashboard-utils.js` when a third dashboard needs them.
- Amber `#f59e0b` is used for Period B (previous year) — consistent with Chart.js palette position 2 across all YoY/WoW dashboards.
- The period picker shows all 12 standard presets; the meaningful ones for this dashboard are `this_month`, `last_month`, `this_quarter` (picks quarter start month), and `custom`.
