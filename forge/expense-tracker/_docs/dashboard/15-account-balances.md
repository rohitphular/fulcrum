...
**File:** `sections/dashboards/15-account-balances.js`
**Group:** Net worth
...
The coordinator does `state.dashChartInstance.destroy()` before switching dashboards. The proxy forwards `.destroy()` to all three Chart instances. The coordinator never needs to know there are multiple charts.
