...
**File:** `sections/dashboards/04-qtd-comparison.js`
...
Uses `computeDailyTotalAssets` (from `dashboard-utils.js`) for both periods — `daysElapsed` data points each.
...
| Utility | Source |
|---|---|
| `filterTxByRange` | `dashboard-utils.js` |
| `groupByDay` | `dashboard-utils.js` |
